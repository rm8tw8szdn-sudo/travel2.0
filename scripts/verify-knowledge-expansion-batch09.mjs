...
import { calculateBatch09ReportData } from "./lib/knowledge-expansion-batch09-report-data.mjs";
...
const evidenceAudit = json("data/knowledge/batches/knowledge-expansion-batch09-evidence-audit.json");
...
const reviews = [1, 2, 3, 4].flatMap((wave) => json(`data/knowledge/batches/review-queue.knowledge-expansion-batch09-wave${wave}.json`).entries);
const stress = json("data/knowledge/reports/knowledge-expansion-batch09-hard-constraint-stress.json");
const browser = json("data/knowledge/reports/knowledge-expansion-batch09-browser-acceptance.json");

assert.equal(stats.seed.sovereignReferenceCount, 195);
...
assert(stats.semanticRepair, "Batch 09 must publish a positive-admission reconciliation audit");
...
assert.equal(stats.additions.pois, stats.semanticRepair.finalBatchPoiAdditions);
...
assert.equal(targetCodes.every((code) => stats.imageManifest.countries.some((record) => record.countryCode === code)), true, "all Batch 09 targets must be Plannable");
...
for (const code of targetCodes) {
  ...
  assert(countryPois.length > 0, `${code}: published route-eligible POI depth`);
...
for (const record of reviews.filter((entry) => entry.entityType === "poi-candidate" && entry.disposition === "quarantined-not-published")) {
  const publishedMatches = pois.filter((poi) => poi.wikidataId === record.wikidataId && poi.parentCityEntityId === record.parentCityEntityId);
  if (record.reasonCodes?.includes("duplicate-wave-poi")) {
    assert(publishedMatches.length <= 1, `${record.wikidataId}: duplicate wave POI published more than once`);
  } else {
    assert.equal(publishedMatches.length, 0, `${record.wikidataId}: quarantined POI leaked`);
  }
}
...
for (const record of routeLegs) {
  ...
  assert(targetCodeSet.has(fromCode) || targetCodeSet.has(toCode), `${record.legEvidenceId}: Batch 09 endpoint missing`);
...
assert.equal(stats.routeConsumption.queryCount, 106);
...
assert.equal(stress.summary.totalCases >= 300, true);
...
assert.equal(stats.images.sealedHistoricalDebt, 13, "the sealed PR #27 debt list must remain immutable historical evidence");
assert.equal(stats.images.totalNeedsBackfillBefore, stats.baseline.images.needsBackfill, "Batch 09 baseline debt must come from its own baseline snapshot");
assert.equal(stats.images.totalNeedsBackfillAfter, stats.images.needsBackfill, "current debt must come from the current image manifest");
assert.equal(stats.images.preBatchScopeNeedsBackfillAfter >= stats.images.sealedHistoricalDebt, true, "semantic withdrawals may increase historical-scope debt but may not erase the sealed debt floor");
...
for (const wave of [1, 2, 3, 4]) {
  const rawPath = `data/knowledge/raw/knowledge-expansion-batch09-wave${wave}.wikidata.json`;
...
console.log(JSON.stringify({
  verifier: "knowledge-expansion-batch09",
  status: "PASS",
  ...
  stressCases: stress.summary.totalCases,
  images: {
