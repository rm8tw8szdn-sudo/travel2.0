...
两个浏览器启动文件共减少 130 行；加上生产入口调整，三个源文件净减少 124 行。测试场景和断言未减少。精简后重新运行：21 项浏览器测试通过（25.4 秒）、22 项单元测试通过、4 项 smoke 检查通过。另行占用启动端口，确认启动失败能被报告且不会留下本轮临时目录；正常运行的临时目录也已确认删除。
