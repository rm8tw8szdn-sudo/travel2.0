...
- Accepted 仓库以带版本、PID、操作系统进程启动身份、创建时间及随机 token 的路径锁串行修改，并在锁内重载最新文件。Windows 通过进程 StartTime、POSIX 通过 `ps` 启动时间区分 PID 复用；查询失败保持 fail-safe。主锁和 recovery marker 仅在完整元数据对应的进程身份可确认已死亡或被替换时恢复，有限 takeover 状态不会递归创建 recovery 链。
...
最终验证：40/40 Node 单元测试、33/33 Chromium 浏览器测试、4/4 smoke 检查通过，依赖审计为 0 个已知漏洞。多城市、多国家、同名城市、生成约束、输入/图片安全及故障传播检查通过。真实 HTTP 回归确认内部 data、raw、reports、audit、LFS 路径及编码/混合分隔符绕过均返回 404，正常页面、assets、vendor 与两个地图数据文件继续返回 200。锁回归确认活跃主锁与 recovery owner 受保护、死亡进程锁和 recovery marker 可恢复、PID 复用可识别、重启可恢复、竞争恢复仍为单写者、畸形元数据及无法判断的进程状态 fail-safe、异常路径正确清锁、不会删除替代 owner 且 A/B/A 写入不丢失。4 项需要本地运行缓存或 Git LFS 原始对象的集成检查在当前部分检出中按新契约明确报告 `BLOCKED`。
