...
assert.deepEqual({ countries: repository.listCountries().length, cities: repository.listCities().length, pois: repository.listPois().length }, { countries: 51, cities: 99, pois: 568 });
...
assert.equal(new Set(allEntities.map((entity) => entity.entityId)).size, 718);
...
assert.equal(new Set(allEntities.map((entity) => entity.wikidataId)).size, 717);
...
console.log(JSON.stringify({
  status: "PASS",
  totals: { countries: 51, cities: 99, pois: 568, entities: 718 },
