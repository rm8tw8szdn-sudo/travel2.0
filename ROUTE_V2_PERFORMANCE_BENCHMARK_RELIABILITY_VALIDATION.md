...
Worker admission is now centralized in `validateWorkerResult()`. The successful schema explicitly requires version 1, `success: true`, `error: null`, exact envelope fields, a zero process exit, no process error/signal/stderr, and operation counts consistent with the fixed protocol. Unknown fields and contradictory states such as valid-looking measurements plus an error marker fail closed before aggregation. The negative matrix establishes the invariant that the combined gate can PASS only when every worker execution and envelope is explicitly valid and successful.

Aggregation now accepts only frozen normalized pairs created by successful `validateWorkerResult()` calls and branded in a module-private `WeakSet`. Raw valid-looking pairs, malformed raw pairs, ordinary `validated: true` fields, copied pairs, and mixed validated/raw collections all return `FAIL / false / INCONCLUSIVE / BLOCKED_INCONCLUSIVE` before statistics. The brand is neither exported nor representable in worker JSON, closing the public aggregation bypass without trusting callers.
