...
export const CITY_BASELINE_P1B_BATCH01_RAW_RELATIVE_PATH = "data/knowledge/raw/cities-p1b-batch01.wikidata.json";
const RAW_PATH = path.join(REPOSITORY_ROOT, CITY_BASELINE_P1B_BATCH01_RAW_RELATIVE_PATH);
const RAW_SCHEMA_VERSION = "route-v2-city-baseline-p1b-batch01-raw";
const ENTITY_API_ENDPOINT = "https://www.wikidata.org/w/api.php";
const SPARQL_ENDPOINT = "https://query.wikidata.org/sparql";
const USER_AGENT = "travel-collection-route-v2-city-baseline-p1b-batch01/1.0";
const BATCH_ALIAS_LANGUAGES = Object.freeze(["en", "zh-hans", "zh", "es", "cs", "fi", "sv", "nl", "pl"]);

export const CITY_BASELINE_P1B_BATCH01_TYPE_QID_POLICY = Object.freeze({
...
export const CITY_BASELINE_P1B_BATCH01_REVIEW_TYPES = Object.freeze([
...
export const CITY_BASELINE_P1B_BATCH01_SEEDS = Object.freeze([
  Object.freeze({ isoAlpha2: "CO", wikidataId: "Q2841", expectedNameEn: "Bogotá", expectedCountryWikidataId: "Q739", parentCountryEntityId: "country-1f698d9652d363bd" }),
  Object.freeze({ isoAlpha2: "CO", wikidataId: "Q48278", expectedNameEn: "Medellín", expectedCountryWikidataId: "Q739", parentCountryEntityId: "country-1f698d9652d363bd" }),
  Object.freeze({ isoAlpha2: "CZ", wikidataId: "Q1085", expectedNameEn: "Prague", expectedCountryWikidataId: "Q213", parentCountryEntityId: "country-a43bbd8228ebbdea" }),
  Object.freeze({ isoAlpha2: "CZ", wikidataId: "Q14960", expectedNameEn: "Brno", expectedCountryWikidataId: "Q213", parentCountryEntityId: "country-a43bbd8228ebbdea" }),
  Object.freeze({ isoAlpha2: "FI", wikidataId: "Q1757", expectedNameEn: "Helsinki", expectedCountryWikidataId: "Q33", parentCountryEntityId: "country-4521f3e0908e41e0" }),
  Object.freeze({ isoAlpha2: "FI", wikidataId: "Q38511", expectedNameEn: "Turku", expectedCountryWikidataId: "Q33", parentCountryEntityId: "country-4521f3e0908e41e0" }),
  Object.freeze({ isoAlpha2: "NL", wikidataId: "Q727", expectedNameEn: "Amsterdam", expectedCountryWikidataId: "Q55", parentCountryEntityId: "country-febe99ab26ea41f0" }),
  Object.freeze({ isoAlpha2: "NL", wikidataId: "Q34370", expectedNameEn: "Rotterdam", expectedCountryWikidataId: "Q55", parentCountryEntityId: "country-febe99ab26ea41f0" }),
  Object.freeze({ isoAlpha2: "PL", wikidataId: "Q270", expectedNameEn: "Warsaw", expectedCountryWikidataId: "Q36", parentCountryEntityId: "country-5a2eabd86d1a1bb1" }),
  Object.freeze({ isoAlpha2: "PL", wikidataId: "Q31487", expectedNameEn: "Kraków", expectedCountryWikidataId: "Q36", parentCountryEntityId: "country-5a2eabd86d1a1bb1" }),
]);

export const CITY_BASELINE_P1B_BATCH01_COUNTRY_ASSET_RELATIVE_PATHS = Object.freeze([
...
export const CITY_BASELINE_P1B_BATCH01_PUBLISH_RELATIVE_PATHS = Object.freeze({
  cities: "data/knowledge/batches/cities.p1b-batch01.json",
  provenance: "data/knowledge/batches/provenance.cities.p1b-batch01.json",
...
function buildSemanticQuery() {
  const values = CITY_BASELINE_P1B_BATCH01_SEEDS.map((seed) => `wd:${seed.wikidataId}`).join(" ");
...
export async function loadKnowledgeCityBaselineP1bBatch01Countries(repositoryRoot = REPOSITORY_ROOT) {
  const assets = await Promise.all(CITY_BASELINE_P1B_BATCH01_COUNTRY_ASSET_RELATIVE_PATHS.map(async (relativePath) => {
...
export function validateKnowledgeCityBaselineP1bBatch01SeedScope({ countries = [], pilotCities = [] } = {}) {
  const seedQids = CITY_BASELINE_P1B_BATCH01_SEEDS.map((seed) => seed.wikidataId);
  const seedIsoAlpha2 = CITY_BASELINE_P1B_BATCH01_SEEDS.map((seed) => seed.isoAlpha2);
  const countryQidList = stableUniqueQids(countries.map((country) => country.wikidataId));
  const countryQids = new Set(countryQidList);
  const pilotCityQids = new Set(pilotCities.map((city) => city.wikidataId));
  const countryQidOverlaps = seedQids.filter((qid) => countryQids.has(qid));
  const pilotCityQidOverlaps = seedQids.filter((qid) => pilotCityQids.has(qid));
  const result = {
    accepted: countries.length === 50
      && CITY_BASELINE_P1B_BATCH01_SEEDS.length === 10
      && new Set(seedQids).size === 10
      && new Set(seedIsoAlpha2).size === 5
      && countryQidOverlaps.length === 0
      && pilotCityQidOverlaps.length === 0,
    countryCount: countries.length,
    seedCount: CITY_BASELINE_P1B_BATCH01_SEEDS.length,
    ...
    countryQidOverlaps,
    pilotCityQidOverlaps,
  };
  return result;
}

export async function refreshKnowledgeCityBaselineP1bBatch01RawSnapshot({
  ...
  if (identityScope?.accepted !== true) throw new Error("city-batch01-identity-scope-not-validated");
  const qids = CITY_BASELINE_P1B_BATCH01_SEEDS.map((seed) => seed.wikidataId);
  ...
  const rawSnapshot = {
    ...
    source: {
      ...
      approvedEntities: CITY_BASELINE_P1B_BATCH01_SEEDS.map((seed) => ({
  ...
  const gate = validateKnowledgeCityBaselineP1bBatch01RawSnapshot(rawSnapshot);
...
export function compareKnowledgeCityBaselineP1bBatch01ClaimProjections(apiClaims = [], sparqlTruthyClaims = []) {
...
function policyMatches(typeQids, policyKey) {
  const policy = new Set(CITY_BASELINE_P1B_BATCH01_TYPE_QID_POLICY[policyKey]);
  return typeQids.filter((qid) => policy.has(qid));
}

export function classifyKnowledgeCityBaselineP1bBatch01TypeQids(typeQids = []) {
...
export function validateKnowledgeCityBaselineP1bBatch01RawSnapshot(rawSnapshot) {
  const results = [];
  const rawEntities = rawSnapshot?.wikidata?.entities || {};
  const semanticEvidence = rawSnapshot?.semanticEvidence || {};
  const expectedQids = CITY_BASELINE_P1B_BATCH01_SEEDS.map((seed) => seed.wikidataId).sort();
  ...
  for (const seed of CITY_BASELINE_P1B_BATCH01_SEEDS) {
    ...
    const claimProjections = {
      P17: compareKnowledgeCityBaselineP1bBatch01ClaimProjections(apiP17, sparqlP17),
      P31: compareKnowledgeCityBaselineP1bBatch01ClaimProjections(apiP31, sparqlP31),
      P131: compareKnowledgeCityBaselineP1bBatch01ClaimProjections(apiP131, sparqlP131),
    };
    const typeClassification = classifyKnowledgeCityBaselineP1bBatch01TypeQids(claimProjections.P31.unionClaims);
    ...
    const knownCountryQids = new Set(rawSnapshot?.source?.identityScope?.countryQids
      || CITY_BASELINE_P1B_BATCH01_SEEDS.map((candidate) => candidate.expectedCountryWikidataId));
...
function createBoundaryReview(type, city, gateResult) {
  ...
  if (!metadata) throw new Error(`unsupported-city-batch01-review-type:${type}`);
...
export function buildKnowledgeCityBaselineP1bBatch01BoundaryReviews({ cities = [], rawGate }) {
  ...
  for (const city of cities) {
    ...
    for (const type of CITY_BASELINE_P1B_BATCH01_REVIEW_TYPES) {
...
export function validateKnowledgeCityBaselineP1bBatch01Isolation({ batchCities = [], pilotCities = [], countries = [] } = {}) {
  const conflicts = [];
  const pilotByEntityId = new Map(pilotCities.map((city) => [city.entityId, city]));
  const pilotByQid = new Map(pilotCities.map((city) => [city.wikidataId, city]));
  const countryByQid = new Map(countries.map((country) => [country.wikidataId, country]));
  for (const city of batchCities) {
    if (pilotByEntityId.has(city.entityId)) {
      conflicts.push(createIsolationConflict("pilot-batch-city-entity-id-overlap", [city.entityId]));
    }
    if (pilotByQid.has(city.wikidataId)) {
      conflicts.push(createIsolationConflict("pilot-batch-city-qid-overlap", [pilotByQid.get(city.wikidataId).entityId, city.entityId], { wikidataId: city.wikidataId }));
    }
...
export function buildKnowledgeCityBaselineP1bBatch01Assets({ rawSnapshot, countries, pilotCities = [] }) {
  const rawGate = validateKnowledgeCityBaselineP1bBatch01RawSnapshot(rawSnapshot);
  const normalized = normalizeKnowledgeCityBaseline({
    rawSnapshot,
    citySeeds: CITY_BASELINE_P1B_BATCH01_SEEDS,
    countries,
  });
  const cities = enrichAliases(normalized.cities, rawSnapshot);
  const deduped = dedupeKnowledgeCityEntities(cities, { countries });
  const isolation = validateKnowledgeCityBaselineP1bBatch01Isolation({ batchCities: cities, pilotCities, countries });
  ...
  const reviewQueue = combineUniqueIssues([
    ...
    ...deduped.reviewQueue,
    ...buildKnowledgeCityBaselineP1bBatch01BoundaryReviews({ cities, rawGate }),
  ], "reviewId");
  ...
  return {
    rawGate,
    citiesAsset: {
      schemaVersion: "route-v2-city-baseline-p1b-batch01",
      generatedFrom: CITY_BASELINE_P1B_BATCH01_RAW_RELATIVE_PATH,
      cityCount: cities.length,
      cities,
    },
    provenanceAsset: {
      schemaVersion: "route-v2-city-baseline-provenance-p1b-batch01",
      generatedFrom: CITY_BASELINE_P1B_BATCH01_RAW_RELATIVE_PATH,
      cityCount: cities.length,
      provenance,
    },
    conflictsAsset: {
      schemaVersion: "route-v2-city-baseline-conflicts-p1b-batch01",
      blockingCount: conflicts.filter((conflict) => conflict.severity === "blocking").length,
      conflictCount: conflicts.length,
      conflicts,
    },
    reviewQueueAsset: {
      schemaVersion: "route-v2-city-baseline-review-p1b-batch01",
...
export function serializeKnowledgeCityBaselineP1bBatch01Assets(assets) {
  return {
...
async function writePublishedAssets(assets) {
  const serialized = serializeKnowledgeCityBaselineP1bBatch01Assets(assets);
  await Promise.all(Object.entries(CITY_BASELINE_P1B_BATCH01_PUBLISH_RELATIVE_PATHS).map(async ([key, relativePath]) => {
...
async function main() {
  ...
  const countries = await loadKnowledgeCityBaselineP1bBatch01Countries();
  const pilotCities = JSON.parse(await readFile(path.join(REPOSITORY_ROOT, PILOT_CITY_RELATIVE_PATH), "utf8")).cities || [];
  const identityScope = validateKnowledgeCityBaselineP1bBatch01SeedScope({ countries, pilotCities });
  if (!identityScope.accepted) throw new Error(`city-batch01-seed-scope-invalid:${JSON.stringify(identityScope)}`);

  if (refresh) {
    const rawSnapshot = await refreshKnowledgeCityBaselineP1bBatch01RawSnapshot({ timeoutMs, retries, identityScope });
    if (rawSnapshot.gate.status !== "PASS") {
      ...
      throw new Error("city-batch01-raw-gate-failed");
    }
    ...
    process.stdout.write(`${JSON.stringify({
      ...
      raw: CITY_BASELINE_P1B_BATCH01_RAW_RELATIVE_PATH,
  ...
  try {
    ...
    if (error?.code === "ENOENT") throw new Error("city-batch01-raw-missing:rerun-with---refresh");
    throw error;
  }
  const assets = buildKnowledgeCityBaselineP1bBatch01Assets({ rawSnapshot, countries, pilotCities });
  ...
  if (assets.citiesAsset.cityCount !== CITY_BASELINE_P1B_BATCH01_SEEDS.length) throw new Error(`city-count-invalid:${assets.citiesAsset.cityCount}`);
  ...
  const unexpectedReviewTypes = [...new Set(assets.reviewQueueAsset.reviewQueue
    ...
    .filter((type) => !CITY_BASELINE_P1B_BATCH01_REVIEW_TYPES.includes(type)))].sort();
  ...
  process.stdout.write(`${JSON.stringify({
    ...
    outputs: CITY_BASELINE_P1B_BATCH01_PUBLISH_RELATIVE_PATHS,
  }, null, 2)}\n`);
}
