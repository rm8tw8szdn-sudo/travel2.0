...
- 静态服务改用公开文件清单，`.git`、`.cache`、环境文件、服务端源码、测试和文档不再可下载。
- 行程及共享弹窗的动态内容统一 HTML 转义；正常表单创建的注入载荷只显示为文字。
- 删除 `reset=empty` 的生产数据清空行为；畸形城市 hash 安全回退。
- Accepted 仓库以路径锁串行修改，并在锁内重载最新文件，避免旧实例覆盖新写入。
...
最终验证：30/30 Node 单元测试、33/33 Chromium 浏览器测试、4/4 smoke 检查通过，依赖审计为 0 个已知漏洞。多城市、多国家、同名城市、生成约束、输入/图片安全及故障传播检查通过。4 项需要本地运行缓存或 Git LFS 原始对象的集成检查在当前部分检出中按新契约明确报告 `BLOCKED`。
